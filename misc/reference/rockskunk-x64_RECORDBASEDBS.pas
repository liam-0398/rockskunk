...
uses
    ...
    regret the refacator but and i am never coming back to this huge complicated monolith style, currently I am devloping much slower due to it. 
    Rockskunk currently doesnt even have records and I want to just get started making the real compiler so I am going the old school way again. 
...
var
    ...
    frameOffset, labelCounter, position, argCount, t_count, linecount, standardLibBytes: Integer;
...
procedure loadRAXVal(v: TValue);
begin
    if (v.vKind = kArray) and (v.vType = vtByte) then
        WriteText('    movzx rax, byte ' + makeParserSpeakASM(v) + #10)
    else
...
function resolveSymbol(name: String): TValue;
var
    i: Integer;
    v: TValue;
begin
    i := findLocalIndex(name);
    if i <> -1 then
        begin
            v.vKind   := kMem;
        ...
        begin
            i := findGlobalIndex(name);
            if i = -1 then
                begin WriteLn(currentLine + '- I CANT BELIEVE YOUVE DONE THIS - RESOLVESYM - UNK SYM >> ' + name); Halt(1); end;
            v.vKind          := kData;
            v.vStringPayload := stateGlobal[i].asmLabel;
            if stateGlobal[i].varType = 'FLOAT' then
                v.vType := vtFloat
            else
                v.vType := vtNumber;
        end;
...
begin
    varToMem := resolveSymbol(variable);
end;
        ...
        begin
            ...
            if (peek() = 'LBRAC') or (peek() = 'LBRACE') or (peek() = 'BANG') then
                begin
                    ...
                    if stateArray[arrayIndex].elementType = vtByte then
                        begin
                            WriteText('    movzx rax, byte [' + stateArray[arrayIndex].asmLabel + ' + rbx]' + #10);
                            argname := 'rax';
                        end
                    else
...
begin
    if peek() = 'AMP' then 
        begin
            consume;
            resolveSyscall := makeParserSpeakASM(emitAddressOf(makeParserSpeakASM(varToMem(consume))));
        end
    else
        begin
            argument := consume;
            if findArrayIndex(argument) <> -1 then
                resolveSyscall := stateArray[findArrayIndex(argument)].asmLabel
            else if isNumber(argument) then
...
function determineArrayValidity(token: String): TValue;
var
    arrayIndex: Integer;
    v, indexVal: TValue;
begin
            ...
            if arrayIndex = -1 then
                begin WriteLn(currentLine + '- UNK ARRAY >> ' + token); Halt(1); end;
...
begin
    ...
    if (peek() = 'LBRAC') or (peek() = 'LBRACE') or (peek() = 'BANG') then
        begin
            arrayValid := determineArrayValidity(token);
            Exit(arrayValid);
        end;
...
begin
    isFloatArg := False;
    WriteLn('EXPRESSION EVAL - RPAR BRANCH (ARGUMENT PARSER)');
    seenFloats := 0;
    seenInts := 0;
    argcounter := 0;
    ...
    repeat
        ...
        if peek() = 'STRING' then
            begin
                argname := consume();
                if Length(argname) = 1 then
                    argname := IntToStr(Ord(argname[1]))   // single char -> immediate ASCII value
                else
                    begin
                        v := emitStringConstant(argname);
                        WriteText('    lea rax, [' + v.vStringPayload + ']' + #10);
                        argname := 'rax';
                    end;
            end
        else
            begin
                ...
                if arrayIndex <> -1 then
                    begin
                        ...
                        if (peek() = 'LBRAC') or (peek() = 'LBRACE') or (peek() = 'BANG') then  // Segfault? !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                            begin
                                v := determineArrayValidity(argname);
             
                                if v.vType = vtByte then
                                    begin
                                        WriteText('    movzx rax, byte ' + makeParserSpeakASM(v) + #10);
                                        argname := 'rax';
                                    end
                                else
                                    argname := makeParserSpeakASM(v);
                            end
                        else
                            argname := stateArray[arrayIndex].asmLabel;
                    end
                else if not isNumber(argname) then
                    argname := makeParserSpeakASM(varToMem(argname));
            end;
...
procedure diDispatch(rightside: TValue; variable: String; arrayIndex: Integer; isFloat: Boolean); 
begin
...
var
    ...
    rightside: TValue;
    a, b, c, num: String;
    existingIndex, functionIndex, arrayIndex, globalIndex: Integer;
...
begin
    ...
    globalIndex := findGlobalIndex(variable);


    if (peek() = 'LBRAC') or (peek() = 'LBRACE') or (peek() = 'BANG') then  // CAUSES SEGFAULT !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        begin
            v := determineArrayValidity(variable);

        consume;                          // eat :=
        rightside := evaluateExpression(v.vType = vtFloat);
        addr := makeParserSpeakASM(v);

        if v.vType = vtFloat then
            emitAssignFloat(addr, makeParserSpeakASM(rightside))
        else if v.vType = vtByte then
            begin
        ...
        begin
            if isDeclared = True then
                begin // turn into something nasm understands instead of just "variable"
                    ...
                    diDispatch(rightside, variable, arrayIndex, isFloat);
                    
                        WriteLn('ASSIGN BRANCH - DECLARED'); // DEBUG
                end
            else if globalIndex <> -1 then
                begin
                    variable := '[' + stateGlobal[globalIndex].asmLabel + ']';
                    isFloat  := (stateGlobal[globalIndex].varType = 'FLOAT');
                    consume(); // :=
                    rightside := evaluateExpression(isFloat);
                    diDispatch(rightside, variable, arrayIndex, isFloat);

                    WriteLn('ASSIGN BRANCH - GLOBAL'); // DEBUG
                end
            else
                begin
            ...
            begin
                if (variable = 'sys') and (peek() = 'LPAR') then
                    begin
                        consume; // (
                        num := resolveSyscall();
                        consume; // ,
...
procedure evaluateAtomicCondition();
var
    token, condition, condlimit, condlimitType: String;
    v: TValue;
    setccInstr: String;
    ateOwnParen: Boolean;
begin
    ...
    if peek() = 'LPAR' then
    begin
        consume;                // this atomic's own opening paren, if the chained syntax was used
        ateOwnParen := True;
    end;

    token := consume;           // left operand

    if (peek() = 'LBRAC') or (peek() = 'LBRACE') or (peek() = 'BANG') then
    begin
        v := determineArrayValidity(token);
        loadRAXVal(v);
    end
    else
        loadRAX(makeParserSpeakASM(varToMem(token)));

    condition := peek;
    consume;                    // eat it

    condlimitType := peek;      // capture the type BEFORE consuming
    condlimit := consume;       // right operand: 0, a var, or a char literal

    if ateOwnParen and (peek() = 'RPAR') then
        consume;                // only ever close a paren this atomic opened

    if condlimitType = 'STRING' then
        condlimit := IntToStr(Ord(condlimit[1]))          // char literal -> ASCII ordinal
    else if condlimitType <> 'NUMBER' then
        condlimit := makeParserSpeakASM(varToMem(condlimit));

    WriteText('    cmp rax, ' + condlimit + #10);

    if condition = 'EQUAL' then setccInstr := 'sete'
    else if condition = 'LESS' then setccInstr := 'setl'
    else if condition = 'LESSEQUAL' then setccInstr := 'setle'
    else if condition = 'MORE' then setccInstr := 'setg'
    else if condition = 'GREQUAL' then setccInstr := 'setge'
    else
    begin
        WriteLn(currentLine + '- I CANT BELIEVE YOUVE DONE THIS - CONDATOM - UNK COMPARATOR >> ' + condition);
        Halt(1);
    end;

    WriteText('    ' + setccInstr + ' al' + #10);   // writes 0 or 1 into AL based on the cmp flags
    WriteText('    movzx eax, al' + #10);           // zero-extends into full rax — setcc only touches the low byte
end;

function condIntrinsic(endlabel: String): Boolean;
var
    connector: String;
...
begin
    ...
    repeat
        ...
        if firstCondition then
            begin
                WriteText('    mov r12, rax' + #10); // accumulator seeded from the first condition
                firstCondition := False;
            end
        else
            begin
                if connector = 'AND' then
                    WriteText('    and r12, rax' + #10)
                else if connector = 'OR' then
                    WriteText('    or r12, rax' + #10);
            end;

        if (peek() = 'AND') or (peek() = 'OR') then
            begin
                connector := peek();
                consume; // eat AND/OR
            end
    ...
    if peek() = 'RPAR' then
        consume; // closes the outer chain-wrapper paren, if the chained syntax left one pending

    WriteText('    cmp r12, 0' + #10);
...
begin
    ...
    frames[frameDepth].loopvaro := loopvar;
    consume; // :=
    loopstart := consume; // 0
    consume; // until
    looplimit := consume;
...
var
    endlabel, chainLabel: String;
begin
    consume; // I
    consume; // LPAR
    endlabel := labelMaker('I');
    chainLabel := labelMaker('IEND');

    condIntrinsic(endlabel);

    frames[frameDepth].kind     := fkCondIf;
    frames[frameDepth].endLabel := endlabel;
    frames[frameDepth].topLabel := chainLabel;
    Inc(frameDepth);
    bodyPending := True;
...
begin
    ...
    begin
        WriteLn(currentLine + '- I CANT BELIEVE YOUVE DONE THIS - ELSE WITH NO IF');
        Halt(1);
    end;

    frames[frameDepth].endLabel := frames[frameDepth].topLabel;
...
var
    ...
    varname, countStr, valStr, asmline: String;
begin
    i := 0;
    position := 0;
    repeat
        case peek() of
            ...
            'RBRACE': begin 
                ...
                if frameDepth > 0 then
                    begin
                        ...
                        case frames[frameDepth].kind of
                            fkLoopF:
                                begin
                            ...
                            fkCondIf:
                                begin
                                    ...
                                    begin
                                        emitLabel(frames[frameDepth].endLabel);
                                        emitLabel(frames[frameDepth].topLabel);
                                    end;
                                end;
                            fkCondElse:
                                emitLabel(frames[frameDepth].endLabel);
                    ...
                    begin
                        ...
                        if peek() = 'LBRAC' then          // qword array
                            ...
                            begin
                                ...
                                WriteLn('REGISTERED ARRAY >> ' + varname + ' idx=' + IntToStr(arrIndex) + ' count=' + IntToStr(stateArrayCount));
                                WriteBSS('    ' + stateArray[arrIndex].asmLabel + ': resb ' +
            ...
            'ASMBLOCK': begin
                ...
                while peek() <> 'PIPE' do
                    begin
                        if position >= t_count then
                            begin
                                WriteLn(currentLine + '- I CANT BELIEVE YOUVE DONE THIS - ASMBLOCK - UNTERMINATED >> missing closing |');
                                Halt(1);
                            end;
                        ...
                        while peek() <> 'TILDE' do
                            begin
                                if position >= t_count then
                                    begin
                                        WriteLn(currentLine + '- I CANT BELIEVE YOUVE DONE THIS - ASMBLOCK - UNTERMINATED LINE >> missing .');
                                        Halt(1);
                                    end;

                                if asmline <> '' then
                                    asmline := asmline + ' ';
                                asmline := asmline + consume;
                            end;

                        consume; // eat the .
                        WriteText('    ' + asmline + #10);
                    end;

                consume; // eat the closing |
            end;
            else
                begin
                    WriteLn(currentLine + '- I CANT BELIEVE YOUVE DONE THIS - PARSER - YOU HAVE FED ME GARBAGE>> ' + peek() + ' ' + peekV() + ' POS->' + IntToStr(position));
                    Halt(1);
                end;
        end;
